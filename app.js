...
function render(focus=true) {
  ...
  if (page==='end') main.innerHTML=`${landscape()}<section class="hero ending"><p class="eyebrow">${escape(pair(stories[current].title))}</p><h1>${t('Εσύ τι λες;','What do you think?')}</h1><p class="intro">${t('Σκέψου την ιστορία που μόλις διάβασες. Είναι OK αυτό που συμβαίνει;','Think about the story you just read. Is what happens OK?')}</p><div class="reaction-feedback" role="status">${reaction ? `<p class="eyebrow">${t('ΑΣ ΤΟ ΔΟΥΜΕ ΛΙΓΟ','LET’S LOOK CLOSER')}</p><p>${escape(pair(stories[current].reflection))}</p>` : ''}</div><section class="help-panel" aria-labelledby="help-title"><h2 id="help-title">${t('Χρειάζεται άμεση βοήθεια;','Need immediate help?')}</h2><a class="police-call" href="tel:100">100 <span>${t('Άμεση Δράση · Ελλάδα','Police emergency · Greece')}</span></a><p>${t('Αν δεν μπορείς να μιλήσεις, στείλε SMS στο 100 με το όνομά σου, την ακριβή διεύθυνση και τι συμβαίνει.','If you cannot speak, text 100 with your name, exact address and what is happening.')}</p><div class="help-links"><a href="sms:100">${t('SMS στο 100','Text 100')}</a><a href="https://www.astynomia.gr/odigos-tou-politi/chrisimes-symvoules/endooikogeneiaki-via/" target="_blank" rel="noopener noreferrer">${t('Πληροφορίες από την ΕΛ.ΑΣ. ↗','Hellenic Police guidance ↗')}</a></div></section><div class="record-link">${button(t('Θέλω απλά να το καταγράψω','I just want to record it'),'report','text-button')}</div>${button(t('Άλλη ιστορία','Another story'),'stories','text-button')}</section><div class="reaction-fabs" role="group" aria-label="${t('Πώς σου φαίνεται αυτό που συμβαίνει;','How do you feel about what happens?')}"><button type="button" lang="en" class="fab fab-ok" data-action="react-ok" aria-pressed="${reaction==='ok'}">it's ok</button><button type="button" lang="en" class="fab fab-not-ok" data-action="react-not-ok" aria-pressed="${reaction==='not-ok'}">it's not ok</button></div>`;
